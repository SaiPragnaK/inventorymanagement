...
class Menu():
    ...
    def make_panel(self):
        ...
        for section in sections:
            ...
            button = ctk.CTkButton(side_panel, text=section.title(), image=img, anchor="w",
                                   font=(self.font, 18), fg_color="transparent", hover_color="#212121",
                                   command=section_functions[section])
    ...
    def shop(self):
        ...
        remove = ctk.CTkButton(self.frame, width=50, command=self.remove_item, text="Remove Item",
...
'''
    ...
    def add_item(self):
        ...
        button = ctk.CTkButton(master=self.win_frame, width=400, text="Add", corner_radius=6,
    ...
    def buy(self):
        ...
        order_id = 1001 if not rows else rows[-1][0] + 1

        self.cur.execute('select order_item_id from order_items;')
        rows = self.cur.fetchall()
        order_item_id = 1 if not rows else rows[-1][0] + 1
    ...
    def make_table(self, col, width, table=None, height=600):
        ...
        for i, value in enumerate(col):
            w = 300 if value == 'Price' else (0 if i == 0 else width)
